...
void	start_drawing_up_down(t_data *data, t_math *math, int x)
{
	...
	while (++y < WINDOW_HIEGHT)
	{
		...
		weight = (math->current_dist - math->dist_player) / (math->dist_wall - math->dist_player);
		math->current_floor_x = weight * math->floor_x_wall + (1.0 - weight) * data->pl.pos_x;
		math->current_floor_y = weight * math->floor_y_wall + (1.0 - weight) * data->pl.pos_y;
